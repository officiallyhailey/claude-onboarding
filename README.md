...
└── website/                                the four guides as a site
```
...
`npm run build` produces a `dist/` folder that opens from disk or from any static
host, with no server configuration, if you would rather hand it over that way.
